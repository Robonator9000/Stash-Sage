...
**Track your cannabis collection, consumption sessions, and stash GÇö with or without an account.**
...
Sign in with email to sync across devices via Supabase. All data backed up to the cloud. Works offline with localStorage as a local cache GÇö synced automatically when reconnected.

| Feature | Without Account | With Account |
|---|---|---|
| All core features | G£à | G£à |
| Local storage | G£à | G£à |
| Cloud backup | GÇö | G£à |
| Cross-device sync | GÇö | G£à |
| Profile & avatar | GÇö | G£à |
| Data export | GÇö | G£à |

### =ƒû+n+Å Profile & Avatars
...
- **Password reset** GÇö forgot password flow sends a reset link
- **Account deletion** GÇö permanently removes all data with a two-step confirmation

### =ƒô+ Strain Color Picker
Click any strain type badge to open a color picker GÇö choose from 10 preset swatches or reset to default. Colors apply instantly to the badge, left-edge highlight, and card glow.
...
### GÜí Low Stock Alerts
Set a gram threshold per product. A toast notification fires when stock drops below it.

### =ƒÆ+ Data Portability
- **JSON export/import** GÇö full backup with merge support
- **CSV export** GÇö spreadsheet-friendly
- **Copy to clipboard** GÇö quick share
- **Manual sync button** GÇö force push/pull from cloud

### =ƒöÆ PIN Lock
4GÇô6 digit PIN screen on app start to secure your data locally.

### =ƒîÉ Multi-Language
English -+ Espa+¦ol -+ Fran+ºais -+ Deutsch -+ Portugu+¬s. Auto-detects browser language.
...
```bash
npm install
npm run dev        # GåÆ http://localhost:5173
```
...
<div align="center">Made with =ƒÆÜ for the community</div>
