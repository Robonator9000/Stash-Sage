...
function formatCurrency(value: number, currency: string): string {
  if (currency === 'EUR') return `Gé¼${value.toFixed(2)}`;
  if (currency === 'GBP') return `-ú${value.toFixed(2)}`;
  if (currency === 'JPY') return `-Ñ${value.toFixed(0)}`;
    ...
    <div className="min-h-screen" style={{ backgroundColor: 'var(--bg)' }}>
      ...
      <header className={`sticky top-0 z-50 ${isDark ? 'bg-[#0b1120]/80' : 'bg-[#e2e8f0]/80'} backdrop-blur-xl`}>
        <div className="max-w-7xl mx-auto px-4 py-2 flex items-center justify-between gap-1.5">
          ...
          >
            <span className="text-2xl bg-gradient-to-r from-cyanx to-emera bg-clip-text text-transparent">=ƒìü</span>
                      ...
                      {filteredHistory.map((entry) => (
                        <tr key={entry.id} className={`transition-colors ${isDark ? 'hover:bg-midnight' : 'hover:bg-gray-50'}`}>
                          ...
                          <td className={`px-4 py-3 text-right ${isDark ? 'text-mist' : 'text-gray-600'}`}>
                            {entry.amount != null ? `${formatPrecision(entry.amount, settings.decimalPrecision)}g` : 'GÇö'}
                          </td>
                          <td className={`px-4 py-3 text-right ${isDark ? 'text-mist' : 'text-gray-600'}`}>
                            {entry.price != null ? formatCurrency(entry.price, settings.currency) : 'GÇö'}
                          </td>
      ...
      {showSmoke && (
        <div className="fixed inset-0 z-[100] pointer-events-none flex items-center justify-center">
          <span className="animate-smoke-puff text-7xl">=ƒÆ¿</span>
          <span className="animate-smoke-puff-2 text-6xl ml-4">=ƒÆ¿</span>
          <span className="animate-smoke-puff text-5xl ml-2" style={{ animationDelay: '0.2s' }}>=ƒÆ¿</span>
        </div>
      )}
      {showDollar && (
        <div className="fixed inset-0 z-[100] pointer-events-none flex items-center justify-center">
