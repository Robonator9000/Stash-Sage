            ...
            <div className="mb-3">
              ...
              <div className="flex items-center gap-2 mt-1">
                <button onClick={() => { const cur = parseFloat(quickSellGrams) || 0; const next = Math.max(0, Math.round((cur - 0.5) * 10) / 10); setQuickSellGrams(next > 0 ? String(next) : ''); }}
                  className={`w-9 h-9 rounded-xl flex items-center justify-center font-bold text-sm transition-colors ${isDark ? 'bg-slate-800 text-slate-400 hover:bg-slate-700 hover:text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>GÍ∆</button>
