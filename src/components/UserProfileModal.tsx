...
export function UserProfileModal({ userId, isDark, lang, onClose }: UserProfileModalProps) {
    ...
    async function load() {
      ...
      if (postsRes.data) {
        const enriched = postsRes.data.map(p => ({
          ...p,
          author: { username: profileRes.data?.display_name || 'Unknown' },
          likes_count: 0,
          liked_by_me: false,
          comments_count: 0,
        }));
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" onClick={onClose}>
      ...
      <div
        className={`relative w-full max-w-lg max-h-[85vh] overflow-y-auto rounded-2xl shadow-2xl ${isDark ? 'bg-card border border-edge' : 'bg-white border border-gray-200'}`}
      ...
      >
        <div className={`sticky top-0 z-10 flex items-center justify-between p-4 border-b ${isDark ? 'bg-card border-edge' : 'bg-white border-gray-200'}`}>
          ...
          <button onClick={onClose} className={`p-2 rounded-xl transition-all ${isDark ? 'text-mist hover:text-frost hover:bg-surface' : 'text-gray-500 hover:text-gray-900 hover:bg-gray-100'}`}>
            <X className="w-5 h-5" />
          </button>
        </div>
